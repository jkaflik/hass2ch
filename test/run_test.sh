...
echo "go run cmd/hass2ch/main.go --pretty-log --host localhost:8123 dump"
echo ""
echo "Or run the full pipeline with:"
echo "go run cmd/hass2ch/main.go --pretty-log --host localhost:8123 pipeline"
echo ""
echo "To run the reconnection test:"
echo "HASS_HOST=localhost:8123 HASS_TOKEN=\"$HASS_TOKEN\" go test -v ./test/..."
echo ""
